  ...
  """
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            if j == y[i]:
                continue
            class_loss = scores[j] - correct_class_score + margin  # note delta = 1
            if class_loss > 0:
                loss += class_loss
                #######################################################################
                # TODO:                                                               #
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  """
    ...
    #############################################################################
    scores = torch.mm(X, W)
    margin = torch.maximum(torch.tensor([0]).cuda(),
                           scores - scores[torch.arange(scores.shape[0]), y].reshape(128, 1) + 1)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
